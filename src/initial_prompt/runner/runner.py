...
from src.initial_prompt.patcher import Patcher

from src.llm import LLM
from src.state import AgentState
from src.project import ProjectManager

PROMPT = open("src/initial_prompt/runner/runner.jinja2", "r").read().strip()
RERUNNER_PROMPT = open("src/initial_prompt/runner/rerunner.jinja2", "r").read().strip()
...
class Runner:
    ...
    def run_code(
        ...
        for command in commands:
            ...
            print(f"Executing command: {' '.join(command_set)} in directory: {project_path}")
            try:
                process = subprocess.run(
                    ...
                    cwd=project_path,
                    shell=True # Use shell=True if necessary
                )
