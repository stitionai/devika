    ...
    def crawl(self):
        ...
        def add_to_hash_tree(hash_tree, tag, node_id, node_name, parent_id):
            ...
            value = (
                (True, node_id)
                if node_name == tag
                else (True, anchor_id) if is_parent_desc_anchor else (False, None)
            )
        ...
        for index, node_name_index in enumerate(node_names):
            ...
            for tag in ancestor_exceptions:
                is_ancestor_of_tag, tag_id = add_to_hash_tree(
                    ancestor_exceptions[tag]["ancestry"],
                    tag,
                    index,
                    node_name,
                    node_parent,
                )
                ancestor_exceptions[tag]["nodes"][str(index)] = (
                    is_ancestor_of_tag,
                    tag_id,
                )
            ...
            is_ancestor_of_anchor, anchor_id = ancestor_exception.get(
                "a", (False, None)
            )
            is_ancestor_of_button, button_id = ancestor_exception.get(
                "button", (False, None)
            )
            ancestor_node_key = (
                str(anchor_id)
                if is_ancestor_of_anchor
                else str(button_id) if is_ancestor_of_button else None
            )
            ancestor_node = (
                ...
                for key, value in element_attributes.items():
                    if ancestor_node is not None:
                        ancestor_node.append(
                            {"type": "attribute", "key": key, "value": value}
                        )
                    else:
            ...
            if (is_ancestor_of_anchor or is_ancestor_of_button) and (
        ...
        for element in elements_in_view_port:
            ...
            should_include_element = (
                inner_text != ""
                or node_name in ["link", "input", "img", "button", "textarea"]
                or (node_name == "button" and meta != "")
            )
            ...
            element_string = (
                f"<{convert_name(node_name, is_clickable)} id={id_counter}{meta}>"
            )
            if inner_text:
                element_string += (
                    f"{inner_text}</{convert_name(node_name, is_clickable)}>"
                )
            else:
                element_string += "/>"
    ...
    try:
        ...
        while True and visits < 5:
            ...
            gpt_cmd = get_gpt_command(
                objective, current_url, prev_cmd, browser_content
            ).strip()
            run_cmd(gpt_cmd)
