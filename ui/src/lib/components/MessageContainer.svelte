			...
			<div class="mt-4 flex items-start space-x-3">
				{#if message.from_devika}
					<img
						...
						alt="Devika's Avatar"
						class="avatar flex-shrink-0 rounded-full"
				...
				<div class="flex w-full flex-col">
					<p class="sender-name text-xs text-gray-400">
						{message.from_devika ? "Devika" : "You"}
						<span class="timestamp">{new Date(message.timestamp).toLocaleTimeString()}</span>
					</p>
					{#if message.from_devika && message.message.startsWith("{")}
						<div class="mr-4 w-full rounded bg-slate-800 p-2" contenteditable="false">
					...
					{:else}
						<div
							class="mr-4 w-full rounded bg-slate-800 p-2"
