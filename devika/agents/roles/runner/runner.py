...
from devika.agents.roles.patcher import Patcher
...
class Runner:
    ...
    def render(self, conversation: str, code_markdown: str, system_os: str) -> str:
    ...
    def run_code(
        ...
        for command in commands:
            ...
            while command_failed and retries < 2:
                new_state = AgentState().new_state()
                new_state["internal_monologue"] = (
                    "Oh seems like there is some error... :("
                )
                new_state["terminal_session"]["title"] = "Terminal"
                new_state["terminal_session"]["command"] = command
                new_state["terminal_session"]["output"] = command_output
                ...
                if action == "command":
                    ...
                    Patcher(base_model=self.base_model).save_code_to_project(
                        code, project_name
                    )
    ...
    def execute(
        ...
        while not valid_response:
            print("Invalid response from the model, trying again...")
            return self.execute(
                conversation, code_markdown, os_system, project_path, project_name
            )
